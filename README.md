...
This query finds top three unique salaries per department.

Approach 1: Using Window Functions (Preferred)
Use DENSE_RANK() to assign a ranking based on salary within each department.
Partition by departmentId to ensure ranking is department-specific.
Filter ranks <= 3 to get top three unique salaries.

Approach 2: Using Subqueries
Select top 3 unique salaries for each department.
Filter employees whose salary is in that top 3 list.
